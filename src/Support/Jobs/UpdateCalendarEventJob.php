...
final class UpdateCalendarEventJob implements ShouldQueue
{
    ...
    public function handle(AppointmentRepository $repo, CalendarGateway $cal, Mailer $mailer): void
    {
        $appt = $repo->findById($this->appointment_id);
        if (!$appt) return;
        $cal->update($appt);
        $mailer->sendUpdated($appt);
    }
}
